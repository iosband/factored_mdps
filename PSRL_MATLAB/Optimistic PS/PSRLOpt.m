...
%PSRLOpt implements the PSRL optimistic algorithm for M episodes of timeframe tau and
...
%The algorithm takes nSample samples and picks optimistically from them
%------------------------------------------------------------------
% M      - 1x1 How many episodes for simulation
% tau    - 1x1 Length of episode
% nSample- 1x1 How many optimistic samples to take
...
for episode=1:M,
    ...
    % Work out the PSRLOpt Policy
    % Collate the necessary Bayes information
    alpha = alpha0 + pEmp; % Transitions
    value = zeros(S,1);
    for iSamp = 1:nSample,
        % Find the optimistic policy out of nSamples
        pSample = sampleDirichletMat(alpha);
        [muSample, varSample] = ...
        sampleNormalGammaMat(mu0,nMu0,tau0,nTau0,vTot,rMean,rVar);
        % Compute the optimum policy for this sampled MDP
        [sValue sPolicy] = dpValueIteration(pSample,muSample,tau);
        
        if(sValue(state) > value(state)),
            % If it's the most optimistic, then we keep it
            policy = sPolicy;
            value = sValue;
        end
    end
        
    % Recording the values/policies chosen
