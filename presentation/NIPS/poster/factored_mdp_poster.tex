...
\vspace{-2mm}
\textcolor{red}{$$ \mathbf{{\rm \textbf{Regret}}(T, M^*) = \tilde{O}\left( \Xi \sum_{j=1}^m  \sqrt{|\Xc[Z^P_j]| \ |\Sc_j| \ T } \right). }$$}
...
\vspace{2mm}
\textbf{Key point:} For $m$ independent components with $S$ states and $A$ actions $ = \tilde{O}(mS\sqrt{AT})$ and close to lower bounds of
$\color{blue}{\underbrace{m\sqrt{SAT)}}_{\text{factored MDP}}}
 \ll \color{green}{\underbrace{\sqrt{(SA)^m T}}_{\text{general MDP}}}$.
}
...
Our analysis allows us to build tight $\Mc_k$ that contain $M^*$ with high probability.
